...
def sign_up():
    if request.method == 'POST':
        ...
        elif len(email) == 0 or len(firstName) == 0 or len(password1) == 0 or len(password2) == 0:
            flash('Plese enter all fields.', category='error')
        elif len(firstName) < 2:
            flash('Name must be greater than 1 character.', category='error')
        elif len(password1) < 6:
            flash('Password must have more than 5 characters.', category='error')
        elif len(password2) == 0:
            flash('Password confirmation must be written.', category='error')
